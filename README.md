...
A production-style REST API wrapping the Telco Customer Churn model in a FastAPI service, so churn predictions can be integrated directly into other systems (a CRM, a scheduled job, a client's own app) rather than only accessed through a UI.

This complements the Streamlit demo built for the same model — the Streamlit app is for a human to interactively explore predictions; this API is for a developer to integrate predictions into their own software.
...
Notes
The 0.70 decision threshold reflects a deliberate business tradeoff (balancing false alarms against missed churners) made and justified during the original model development — see the churn analysis repo for the full reasoning.
